...
class NewsRepository(private val database: ArticleDatabase) {
        ...
        withContext(Dispatchers.IO) {
            ...
            if (currentArticles.isEmpty() || (System.currentTimeMillis() - currentArticles[0].createdAt) > ONE_HOUR_IN_MILLIS) {
                ...
                if (response.isSuccessful) {
                    Log.d("newsRepository", "refreshNews: ${response.isSuccessful}")
                    Log.d("newsRepository", "refreshNews: ${response.code()}")
                    response.body()?.let { newsResponse ->
                        Log.d("newsRepository", "refreshNews: INSERT_CALLED")
                    ...
                    Log.e("newsRepository", "BAD_API_CALL: ${response.code()}")
                }
    ...
    suspend fun getSearchedNews(searchQuery: String, pageNumber: Int) =
        NewsApi.retrofitService.getSearchedNews(searchQuery, pageNumber = pageNumber)
